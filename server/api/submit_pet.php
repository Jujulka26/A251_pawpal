<?php
    ...
    if (isset($_POST['image_paths'])) {
        ...
        foreach ($image_paths as $index => $base64_image) {
            ...
            $filenames[] = $filename;
            $decoded_images[] = base64_decode($base64_image);
        }
    }
	...
	VALUES ('$user_id','$pet_name','$pet_type','$category','$description', '$image_paths_string', '$lat', '$lng')";
	try{
		if ($conn->query($sqlinsertservice) === TRUE){
            for ($i = 0; $i < count($filenames); $i++) {
                $filename = $filenames[$i];
                $decodedImage = $decoded_images[$i];
